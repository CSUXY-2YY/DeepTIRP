...
def training_Efficientnet_B3():
    prddir = 'Efficientnet_B3_weights/GorC_2_label/'
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'validation']:
            ...
            for inputs, labels, _, _, _, _ in dataloaders[phase]:
            ...
            if phase =='validation':
                if epoch_acc>best_acc:
                    torch.save(model, 'Efficientnet_B3_weights/GorC_2_label/best_weight_E_B3')
                    best_acc = epoch_acc
    ...
    # %%
    torch.save(model, 'Efficientnet_B3_weights/GorC_2_label/best_weight_E_B3_final')

    # %%
    model = torch.load('Efficientnet_B3_weights/GorC_2_label/best_weight_E_B3_final')
    model.to(device);
    ...
    all_image_name = []
    running_corrects = 0
    for inputs, labels1, _, _, _, img_name in dataloaders['test']:
        inputs = inputs.to(device)
    ...
    # %%
    np.save('results/E_B3_result',final_result)

if __name__ == '__main__':
    
    training_Efficientnet_B3()
